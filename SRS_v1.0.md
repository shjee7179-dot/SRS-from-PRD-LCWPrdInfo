...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|:---:|:---:|:---|:---|
| 0.2 | 2026-04-15 | 초기 작성 — PRD(VPS v1.0) 기반 전체 SRS 생성 | Senior Requirements Engineer |
| 0.3 | 2026-04-21 | 비용/효율성 검토 보고서 반영 (MVP 개발속도 최적화, 외부연동 최소화, Supabase 통제 강화) | AI Consultant |
...
| C-TEC-008 | 세션 인증 및 역할 기반 접근 제어(RBAC)는 자체 구현을 배제하고 100% Supabase Auth 및 RLS(Row Level Security)를 활용해 개발 속도를 극대화 | 인증 로직 효율화 (지연 요인 제거) |
...
> Vibe Coding 최적화를 위해 내부 API를 Next.js Server Actions(`"use server"`)로 대체하고 외부 연동에만 Route Handlers(`/api/*`)를 사용합니다. 단, 향후 마이그레이션(Vendor Lock-in 방어)을 대비해 비즈니스 로직은 UI 계층과 철저히 분리하여 `actions/` 폴더에 격리 구성합니다.
...
> 단일 프레임워크 기반 바이브코딩 최적화 아키텍처입니다. 불필요한 마이크로서비스 노드를 제거하고 Next.js 구조로 통폐합하되, CSAP 인증 대비 및 클라우드 이전 확장을 위해 코어 비즈니스 로직 모듈의 철저한 디렉터리 분리를 원칙으로 합니다.
...
| REQ-FUNC-002 | 시스템은 매핑 완료를 위한 AI 자동 분석 제안 기능을 제공해야 한다. (MVP 단계: 완벽한 AI 정확도보다 Human-in-the-loop 방식의 수동 보정 유연성에 집중) | JTBD-1 / F1 / P1-1 | Must | **Given** 비표준 컬럼 파일이 업로드되었을 때 **When** AI 매핑 프로세스가 실행되면 **Then** 시스템은 수 초 이내에 제안안을 도출하며, 예상치 못한 구조라도 관리자가 직관적으로 직접 수정할 수 있는 편집 뷰(REQ-FUNC-003)로 즉시 전환된다. |
...
| Req ID | 요구사항 | Source | Priority | Acceptance Criteria |
|:---|:---|:---|:---:|:---|
| REQ-FUNC-019 | 시스템은 향후 IMS Open Badges 2.0 표준 연동을 대비하여 Local Fit Badge 발급 데이터를 표준 JSON 규격(Assertion)으로 변환 후 자체 DB(Supabase)에 격납해야 한다. (MVP 단계: 직접적인 외부망 연동 배제) | JTBD-2 / F4 / P2-3 | Must | **Given** 학습자가 Local Fit 기준을 충족했을 때 **When** Badge 발급 프로세스가 실행되면 **Then** IMS OB 2.0 표준에 대비한 JSON Assertion이 생성되고, 외부 전달 로직 없이 자체 프로필 데이터베이스에만 기록된다. |
...
| REQ-FUNC-021 | 시스템은 발급된 Badge의 진위를 제3자가 검증(Verification)할 수 있는 자체 플랫폼 내 공개 페이지를 제공해야 한다. | F4 | Must | **Given** Local Fit Badge가 발급되었을 때 **When** 기업 HR 담당자가 Badge 검증 URL에 접근하면 **Then** 시스템 자체 DB를 조회하여 해당 Badge 발급 기관, 발급일, 충족 기준, 유효 여부를 즉시 웹으로 표시한다. |
...
| Req ID | 요구사항 | Source | Priority | Acceptance Criteria |
|:---|:---|:---|:---:|:---|
| REQ-FUNC-038 | 시스템은 커스텀 세션 구현을 배제하고 Supabase Auth 모듈 및 PostgreSQL RLS를 통제 수단으로 하는 100% BaaS 기반 권한(RBAC) 제어를 강제해야 한다. | 전체 | Must | **Given** "사업관리 매니저" 역할의 사용자가 로그인했을 때 **When** 인재 열람에 접근하면 **Then** API를 우회하더라도 DB 엔진 레벨(RLS)에서 조회가 원천 차단되고 권한 부족이 반환된다. |
